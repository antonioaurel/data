...
(Codex) review orchestration**, a per-PR **review and impact trace**, and the
**app impact** of each PR. The sections are cross-linked — click any `#PR` chip
(or an impact count) to highlight that PR across all of them.
...
| **Review & Impact Trace** | Task table + PR comments (`gh`) + impact matrix | Per-PR validated exchange summary: what Claude implemented, what Codex reviewed, what was fixed/replied, and the impacted areas mapped from the matrix. |
...
- **4 · Review & Impact Trace** — per PR, a compact timeline of Claude/Codex exchanges and a named list of impacted areas.
- **5 · Impacts** — per PR, the functionalities it touches, grouped by subsystem (Desktop / Mobile / Platform).
